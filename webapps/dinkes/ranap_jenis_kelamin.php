...
<html>
    ...
    reportsqlinjection();      
        ...
        $_sql = "SELECT setting.kode_ppk, setting.nama_ppk, pasien.jk, Count(pasien.jk) AS jumlah_pasien
				 FROM pasien , reg_periksa , penjab , poliklinik , setting , kamar_inap
				 WHERE pasien.no_rkm_medis = reg_periksa.no_rkm_medis AND reg_periksa.kd_poli = poliklinik.kd_poli AND reg_periksa.kd_pj = penjab.kd_pj AND reg_periksa.no_rawat = kamar_inap.no_rawat and
				 pasien.jk = pasien.jk AND kamar_inap.tgl_masuk BETWEEN '$tanggal1' AND '$tanggal2'
				 GROUP BY pasien.jk";            
