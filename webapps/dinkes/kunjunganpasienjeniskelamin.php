    ...
    foreach($_instalasi as $instansi) {
			...
			foreach($_jenis as $jns_kelamin)
		{
			$_sql = "SELECT Count(pasien.jk) AS jumlah_pasien
				 FROM reg_periksa JOIN penjab ON reg_periksa.kd_pj = penjab.kd_pj JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis WHERE reg_periksa.kd_poli = 'IGD' AND jk='$jns_kelamin' AND tgl_registrasi  BETWEEN '$tanggal1' AND '$tanggal2'";            
