    ...
    foreach($_instalasi as $instansi) {
			...
			foreach($_jenis as $jns_byr)
		{
			$_sql = "SELECT Count(reg_periksa.kd_pj) AS jumlah_pasien
				 FROM reg_periksa JOIN penjab ON reg_periksa.kd_pj = penjab.kd_pj WHERE reg_periksa.kd_poli = 'IGDK' AND png_jawab='$jns_byr' AND tgl_registrasi  BETWEEN '$tanggal1' AND '$tanggal2'";            
