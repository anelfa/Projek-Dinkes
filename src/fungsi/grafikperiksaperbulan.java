    ...
    public static CategoryDataset createDataset1(String symbol) { //data grafik nilai K dan D
          ...
          String series2 = "Bulan";
            ...
            try {
                ...
                ResultSet rs = stat.executeQuery("select DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m'),count(DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m')) as jumlah "+
                   "from reg_periksa  "+
                   symbol+" group by DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m')");
                while (rs.next()) {
