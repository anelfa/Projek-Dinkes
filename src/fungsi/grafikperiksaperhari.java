    ...
    public static CategoryDataset createDataset1(String symbol) { //data grafik nilai K dan D
          ...
          String series2 = "Tanggal";
            ...
            try {
                ...
                ResultSet rs = stat.executeQuery("select DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m-%d'),count(DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m-%d')) as jumlah "+
                   "from reg_periksa  "+
                   symbol+" group by DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m-%d')");
                while (rs.next()) {
